package $7genericCreatingClassesAndMethod.$1genericadvantage;

public class Main {

    /**
     * Recognizing Generic and Environment Setup Advantages
     *  -   Generics: Enables types (interfaces and classes) to be parameters while defining
     *      classes, interfaces, methods
     *  -   Advantages of Generics:
     *      -   Runtime checks become compile time checks
     *      -   Eliminates casts while accessing data
     *      -   Allow extensive code reuse making code maintenance easier
     *      -   Allow programmers to use generic algorithms
     */

    public static void main(String[] args) {



    }

}
